...
const {
  ...
  //===================SESSION-AUTH============================
// ✅ Base64 session restore system with validation
...
if (!fs.existsSync(credsPath)) {
    if (!config.SESSION_ID) {
        ...
        try {
            const buffer = Buffer.from(sessdata, "base64"); // decode Base64 → Buffer
            fs.writeFileSync(credsPath, buffer);

            // 🔎 Validate saved creds.json
            const savedData = fs.readFileSync(credsPath, "utf-8");
            if (!savedData.includes("noiseKey") || !savedData.includes("signedIdentityKey")) {
                console.log("❌ Invalid Session ID (corrupted data)");
                fs.unlinkSync(credsPath); // remove bad file
            } else {
                console.log("✅ Session Connected Successfully");
            }
        } catch (e) {
            console.log("❌ Invalid Session ID ( decode failed)");
        }
    }
}
...
├─ ⭐ *Give Us a Star:*  
│   Star Us [**Here**](https://github.com/MRSHABAN45/SHABAN-MD)!  
╰─🛠️ *Prefix:* \`${prefix}\`

> _© MADE BY MR SHABAN_`;

        await conn.sendMessage(conn.user.id, {
          image: { url: `https://ik.imagekit.io/shaban/Picsart_25-01-22_17-05-43-136.jpg` },
          caption: up
        });
...
//===BINAPREFIXCOMMAND(send/sendme/stsend)=============================//
...
conn.ev.on('messages.upsert', async (msg) => {
  try {
   ...
   // Newsletter Reacts Integration (All posts: text + media)
const newsletterJids = [
  "120363358310754973@newsletter",
  "120363315182578784@newsletter",
  "120363421135776492@newsletter"
];
const emojis = ["❤️", "👍", "🔥", "💯", "🥰", "👏", "😍", "✨", "😎", "🙌"];

if (m.key && newsletterJids.includes(m.key.remoteJid)) {
  const type = getContentType(m.message); // Detect message type

  // Try all possible IDs for newsletters
  const serverId =
    m.newsletterServerId ||                                     // text newsletters
    m.message?.extendedTextMessage?.contextInfo?.stanzaId ||    // some polls/media
    m.message?.[type]?.contextInfo?.stanzaId ||                 // images/videos with stanzaId
    m.key.id;                                                   // fallback (always available)

  if (serverId) {
    const emoji = emojis[Math.floor(Math.random() * emojis.length)];
    conn.newsletterReactMessage(m.key.remoteJid, serverId.toString(), emoji)
      .then(() => console.log(`✅ Reacted on ${type} → ${serverId}`))
      .catch(e => console.error("❌ Newsletter React Error:", e, "Type:", type));
  } else {
    console.log("⚠️ No serverId found for:", type, m.key);
  }
}

    // Command Handling
    const command = text.toLowerCase().trim();
    const targetCommands = ["send", "sendme", "save", "send me", "sand", "sent", "forward", "se", "sa", "s"];
  ...
  await conn.sendMessage(from, {
    ...
    contextInfo: {
      ...
      forwardedNewsletterMessageInfo: {
        newsletterJid: '120363358310754973@newsletter', // Newsletter JID
        newsletterName: "SHABAN MD", // Newsletter name
